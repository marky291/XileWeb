...
use App\WoeEvents\WoeEventScoringLeaderboard;
use App\WoeEvents\WoeEventScoringLongestHeldCastle;
use App\WoeEvents\WoeEventScoringWinningGuild;
...
class ProcessWoeEventPoints
{
     ...
     * @throws \Throwable
     */
    public function handle(string $castle)
    {
        $currentSeason = $season ?? now()->format('n');

        $this->scoreRecorder->castle = $castle;
        $this->scoreRecorder->season = $currentSeason;

        $events = $this->fetchEvents($castle, $currentSeason);
    ...
    /**
     * Fetches events for a given castle and season.
     */
    private function fetchEvents(string $castle, int $season)
    {
        ...
        DB::transaction(function () use ($guildDurations, $guildAttended, $season, $events, $castle) {
            ...
            foreach (array_unique($mergedGuilds) as $guild_id) {
                ...
                if (isset($guildDurations[$guild_id]) && $guild_id == $longestDurationGuildId) {
                    ...
                    $this->scoreRecorder->longest_hold_guild = $guild;
                ...
                if ($firstBreakGuild && $firstBreakGuild->guild_id === $guild_id) {
                    ...
                    $this->scoreRecorder->first_break_guild = $guild;
                ...
                if (isset($guildAttended[$guild_id])) {
                    ...
                    $this->scoreRecorder->addAttendee($guild);
                ...
                if ($winningGuildEvent && $guild_id == $winningGuildEvent->guild_id) {
                    ...
                    $this->scoreRecorder->winning_guild = $guild;
